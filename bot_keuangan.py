...
# ================= HELPER =================
def parse_tanggal(input_text):
    today = datetime.now()
    if input_text.lower() == "hari ini":
        return today.strftime("%Y-%m-%d")
    if input_text.lower() == "kemarin":
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("mode") == "pm_tanggal":
        ...
        if not tanggal:
            ...
            return
        no = generate_no_transaksi("pemasukan")
        cursor.execute(
    ...
    if context.user_data.get("mode") == "pg_tanggal":
        ...
        if not tanggal:
            await update.message.reply_text("Format tanggal salah.")
            return
        no = generate_no_transaksi("pengeluaran")
        cursor.execute("""
    ...
    if text == "Grafik Kategori":
        ...
        plt.close()
        await update.message.reply_photo(open("grafik.png","rb"))
